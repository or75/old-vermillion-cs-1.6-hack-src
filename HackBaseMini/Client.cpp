	...
	if (FirstFrame)
	{
		...
		if (PUBLIC_VERSION)
		{
			...
			if (VOVAN_VERSION == 0)
			{
				...
				g_pEngine->pfnConsolePrint("  Vermillion [PUBLIC VERSION] \n");
				g_pEngine->pfnConsolePrint("  by malanris & poiuytrew \n\n");
				g_pEngine->pfnConsolePrint("  Credits:\n");
				g_pEngine->pfnConsolePrint("  - AntiValve\n");
				g_pEngine->pfnConsolePrint("  - PRONAX\n");
				g_pEngine->pfnConsolePrint("  - _or_75\n");
				g_pEngine->pfnConsolePrint("  - Ve4niySooN\n");
				g_pEngine->pfnConsolePrint("  - b15q95\n");
				g_pEngine->pfnConsolePrint("  - Others who help me :)\n\n");
			}
		}
		else
		{
...
void HUD_Redraw(float time, int intermission)
{
	...
	if (g_AntiScreen.DrawVisuals)
	{
		if (PUBLIC_VERSION && VOVAN_VERSION == 0 && AIM_VERSION == 0 )
			g_Drawing.DrawString(5, 130, 205, 0, 0, "Vermillion [PUBLIC]");
